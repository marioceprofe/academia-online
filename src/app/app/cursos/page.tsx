...
function CursoCard({ curso }: { curso: Curso }) {
  ...
  return (
    <Link href={`/cursos/${curso.id}`} className="card group hover:shadow-lg hover:-translate-y-1 transition-all duration-200">
      <div className="h-36 bg-gradient-to-br from-brand-50 to-brand-100 flex items-center justify-center relative overflow-hidden">
        ...
        {isPremium && (
          <div className="absolute top-2.5 right-2.5 flex items-center gap-1 bg-ink-900/80 text-white text-xs px-2 py-1 rounded-lg font-display font-medium">
            <Lock className="w-3 h-3" />Premium
          </div>
        )}
        {!isPremium && <div className="absolute top-2.5 right-2.5 badge-success text-xs">Gratis</div>}
      </div>
      <div className="p-4">
        <h3 className="font-display font-semibold text-ink-900 text-sm leading-snug mb-2 group-hover:text-brand-700 transition-colors line-clamp-2">
          {curso.titulo}
        </h3>
        {curso.descripcion && (
          <p className="text-xs text-ink-400 font-body line-clamp-2 mb-3 leading-relaxed">{curso.descripcion}</p>
        )}
        <div className="flex items-center justify-between pt-2 border-t border-ink-100">
          <span className="text-xs text-ink-400 font-body flex items-center gap-1"><BookOpen className="w-3 h-3" /> Ver curso</span>
...
function EmptyState() {
  return (
    <div className="text-center py-20">
      ...
      <p className="text-ink-500 font-body text-sm max-w-xs mx-auto">Estamos preparando el contenido. Regístrate para recibir un aviso cuando estén disponibles.</p>
      <Link href="/auth/registro" className="btn-primary mt-6 inline-flex">Avisarme</Link>
    </div>
  )
}
