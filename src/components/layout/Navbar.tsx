        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <span className="font-display font-semibold text-ink-900 text-lg tracking-tight">
              Academia<span className="text-brand-600">Pro</span>
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map(link => (
              <Link
                ...
                href={link.href}
                className={cn(
                  'px-4 py-2 rounded-lg text-sm font-display font-medium transition-colors',
                  pathname === link.href
          ...
          <div className="md:hidden py-4 border-t border-ink-100 space-y-1">
            {navLinks.map(link => (
              <Link
                ...
                className={cn(
                  'block px-4 py-3 rounded-xl text-sm font-display font-medium',
                  pathname === link.href
                    ? 'bg-brand-50 text-brand-700'
            ...
            <div className="pt-3 flex flex-col gap-2 px-1">
              <Link href="/auth/login" className="btn-secondary w-full" onClick={() => setMenuOpen(false)}>
                Entrar
              </Link>
              <Link href="/auth/registro" className="btn-primary w-full" onClick={() => setMenuOpen(false)}>
                Empezar gratis
              </Link>
            </div>
          </div>
